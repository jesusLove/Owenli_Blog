...
.macro CheckMiss // 缓存未命中，查找方法列表
	// miss if bucket->sel == 0
.if $0 == GETIMP
	cbz	x9, LGetImpMiss
.elseif $0 == NORMAL
	cbz	x9, __objc_msgSend_uncached // 超找方法列表 <<<<<<<< 447行
.elseif $0 == LOOKUP
...
1:	cmp	x9, x1			// if (bucket->sel != _cmd)
	b.ne	2f			//     scan more
	CacheHit $0			// call or return imp 缓存命中, 返回IMP <<<<<<<<< 缓存命中203行
	
2:	// not hit: x12 = not-hit bucket
	CheckMiss $0			// miss if bucket->sel == 0 ， 未命中
...
LGetIsaDone:
	CacheLookup NORMAL		// calls imp or objc_msgSend_uncached，查找缓存 <<<<<<<<<<<<<<<<<<< 缓存查找244行
...
// MethodTableLoopup  ----> _class_lookupMethodAndLoadCache3
.macro MethodTableLookup // 查找方法表的宏
	...
	mov	x2, x16
	bl	__class_lookupMethodAndLoadCache3 // 调用C语言函数，注意需要去掉一个下划线（objc-runtime-new.mm）中
...
.endmacro
	...
	MethodTableLookup //方法表中查找 <<<<< 查找方法表的宏
	br	x17
