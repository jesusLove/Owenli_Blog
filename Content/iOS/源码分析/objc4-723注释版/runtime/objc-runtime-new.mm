...
static method_t *search_method_list(const method_list_t *mlist, SEL sel)
{
    ...
    if (__builtin_expect(methodListIsFixedUp && methodListHasExpectedSize, 1)) {
        return findMethodInSortedMethodList(sel, mlist); // 排序的方法列表
    } else {
        // Linear search of unsorted method list 线性查找，未排序的方法列表
        for (auto& meth : *mlist) {
                       ...
                       bool initialize, bool cache, bool resolver)
{
 ...
 retry:    
    ...
    //  尝试在查找一般缓存，之前虽然查找过，防止动态添加了在查找一遍 ①
    imp = cache_getImp(cls, sel);
    if (imp) goto done;

    // Try this class's method lists. 尝试在方法列表中查找
    {
        Method meth = getMethodNoSuper_nolock(cls, sel); // 方法查找： 如果是排序使用二分查找，未排序的则使用线性查找 ②
        if (meth) {
            // 填充缓存，将查找的方法添加到缓存中。
    ...
    // Try superclass caches and method lists. 查找父类缓存和方法列表 ③
    {
             ...
             curClass = curClass->superclass)
        {
            ...
            // Superclass cache.
            imp = cache_getImp(curClass, sel); // 查找缓存
            if (imp) {
                if (imp != (IMP)_objc_msgForward_impcache) {
                    // Found the method in a superclass. Cache it in this class.
                    log_and_fill_cache(cls, imp, sel, inst, curClass); // 添加消息接收者的缓存中。
                    goto done;
                }
                else {
    ...
    if (resolver  &&  !triedResolver) {
        runtimeLock.unlockRead();
        _class_resolveMethod(cls, sel, inst); // 动态方法解析，分为Instance和Class两种
        runtimeLock.read();
        // Don't cache the result; we don't hold the lock so it may have 
        // changed already. Re-do the search from scratch instead.
        triedResolver = YES; //标记为解析
        goto retry; // 再次进入消息发送阶段，所以动态添加的方法也会缓存。
    }
