...
// 切面容器
// 通过NSObject分类中的AssociatedObject方法与当前要Hook的目标关联在一起的。
@interface AspectsContainer : NSObject
- (void)addAspect:(AspectIdentifier *)aspect withOptions:(AspectOptions)injectPosition;
- (BOOL)removeAspect:(id)aspect;
- (BOOL)hasAspects;
// 用来关联AspectOption中指定的三种切面。
...
/*
 1. 对对象的forwardInvocation进行Swizzling
 2. 另一个对传入的selector进行Swizzling
 */
static void aspect_prepareClassAndHookSelector(NSObject *self, SEL selector, NSError **error) {
    NSCParameterAssert(selector);
    // 1. 动态生成一个子类，并hookforwardInvocation方法
...
/*
 swizzling类/对象的forwardInvocation函数，对于对象而言，
 并没有直接swizzlinge对象的forwardInvocation方法，而是动态生成一个子类
 将对象与子类关联，然后替换子类的forwardInvocation方法(object_setClass(self, subclass))
 将当前对象的isa指针指向subclass,同时修改subclass以及其subclass metaclass的class方法，使他返回当前对象的class
 */
static Class aspect_hookClass(NSObject *self, NSError **error) {
	...
	if ([className hasSuffix:AspectsSubclassSuffix]) {
		return baseClass;
        // 我们混写了一个class对象，而不是一个单独的Object
	}else if (class_isMetaClass(baseClass)) {
        // baseClass是元类，则self是Class或者MetaClass，混写Self
        return aspect_swizzleClassInPlace((Class)self);
        // Probably a KVO'ed class. Swizzle in place. Also swizzle meta classes in place.
        // 可能是一个KVO class ，混写就位。也要混写meta classes
    }else if (statedClass != baseClass) {
        // 当.class和isa指向不同时，混写baseClass
    ...
    // 3. ISA不存在，动态创建一个子类。
	if (subclass == nil) {
        // 3.1 baseClass作为新类的superClass，类名为subclass
        ...
        // 3.3 subClass.class = statedClass
...
static void aspect_swizzleForwardInvocation(Class klass) {
    ...
    // 如果没有method, replace和class_addMethod一样
...
/// 担当记录已混写类e的角色，内部结构 <NSStringFromClass(Class)>
/// 修改SwizzledClasses集合的方法
/// 记录了forwardInvocation:被混写的类名称。
...
/// 已经混写类字典的角色，内部结构 <Class: AspectTracker *>
/// 该方法用来访问全局变量
/// 记录了整个Hook影响的Class包含其中SuperClass的追踪记录的全局字典。
...
// 判断能够被Hook
// @"retain", @"release", @"autorelease", @"forwardInvocation:"e等方法外部是不可以被hook的，另外还有继承关系中，只能被hook一次
static BOOL aspect_isSelectorAllowedAndTrack(NSObject *self, SEL selector, AspectOptions options, NSError **error) {
    ...
    // 1. 当前实例的类是否是元类;也就是判断self是对象实例，还是类实例
